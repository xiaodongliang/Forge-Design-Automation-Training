...
namespace MyTestDesignAutomation
{
    class VariousInputs
    {
        static public void VariousInputsMain()
        {
            ...
            //测试创建使用不同输入参数的Activity
            CreateOneActivity_withVariousInput(container, myAct_with_Pack_Name, myAppPackName);
            //执行任务 （ WorkItem），使用不同输入参数
            CreateWorkItem_withVariousInput(container, myAct_with_Pack_Name);
        }
        ...
        /// 创建某Activity,使用不同类型的输入参数
        static Activity CreateOneActivity_withVariousInput(Container container,
                                                            string actId,
                                                            string appPackName)
        {
            Console.WriteLine("正在创建某关联AppPackage的Activity，使用不同类型的输入参数...");
            ...
            var activity = new Activity()
            {
                ...
                {
                    //普通测试
                    Script = "MyPluginCommand EmbeddedParams.json LinkedParams.json _save result.dwg\n"
                ...
                Parameters = new Parameters()
                {
                    InputParameters =
                    {
                        ...
                        new Parameter()
                        {
                          Name = "EmbeddedParams", LocalFileName = "Embeddedparams.json"
                        },
                        new Parameter()
                        {
                          Name = "LinkedParams", LocalFileName = "Linkedparams.json"
                        }
                     },
                    OutputParameters = {
                    new Parameter()
        ...
        //创建Work Item
        static void CreateWorkItem_withVariousInput(Container container, string actId)
        {
